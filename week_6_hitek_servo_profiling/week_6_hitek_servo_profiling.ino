...
void loop() {
   ...
   if (rotate_init)
   {
     servo_test_1.write(start_pos);
     servo_test_2.write(start_pos);
     rotate_init = false;
     rotate_wait = true;
   }
   ...
   if ((read_enable) && (currUpdateMillis - prevUpdateMillis >= update_time))
   {
     ...
     servo_test_1.write(positions[update_counter]);
     servo_test_2.write(positions[update_counter]);
     update_counter++;
